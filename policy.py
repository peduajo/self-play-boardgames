...
class CombinedModule(nn.Module):
    def __init__(self, feature_extractor, mlp_extractor, value_net, action_net):
    ...
    def forward(self, x):
        ...
        latent_pi, latent_vf = self.mlp_extractor(x1)
        p = self.action_net(latent_pi)
        v = self.value_net(latent_vf)
        return p, v
...
class ActorCriticPolicy(BasePolicy):
    ...
    def export_tensorrt(self, input_shape, path):
        combined_module = CombinedModule(self.features_extractor,
                                                self.mlp_extractor,
                                                self.value_net,
                                                self.action_net)

        combined_module.eval()
        compile_spec = {"inputs":[torch_tensorrt.Input(input_shape)],
                    "enabled_precisions": torch.half,
                    "workspace_size" : 1 << 22
                    }
        model = torch_tensorrt.compile(combined_module, **compile_spec)
        model.save(path)
